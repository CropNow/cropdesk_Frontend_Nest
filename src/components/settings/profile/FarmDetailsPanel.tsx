...
export function FarmDetailsPanel() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setFarms(farms.map(f => (f.id === selectedFarmId || f._id === selectedFarmId) ? { ...f, name: formData.name, location: { ...f.location, address: formData.addressLine, city: formData.city, state: formData.state, country: formData.country, zipCode: formData.zipcode }, soilType: formData.soilType, irrigationType: formData.irrigationType, farmingType: formData.farmingType } : f));
    } catch (err: any) {
