...
export function CropDetailsPanel() {
          ...
          data.forEach(d => {
            if (Array.isArray(d.crops)) {
              d.crops.forEach((c: any) => {
                if (c && !extractedCrops.some(existing => (existing.id || existing._id) === (c.id || c._id))) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setCrops(crops.map(c => (c.id === selectedCropId || c._id === selectedCropId) ? { ...c, name: formData.name, plantingDate: formData.plantingDate, expectedHarvestDate: formData.expectedHarvest, area: formData.area } : c));
    } catch (err: any) {
      ...
      {crops.length > 0 ? (
        <div className="space-y-4 pt-4 border-t border-cardBorder">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="space-y-2">
              <span className="text-sm font-semibold text-textMuted uppercase">Planting Date *</span>
              <input
                type="date"
            ...
            <label className="space-y-2">
              <span className="text-sm font-semibold text-textMuted uppercase">Expected Harvest *</span>
              <input
                type="date"
                value={formData.expectedHarvest ? new Date(formData.expectedHarvest).toISOString().split('T')[0] : ''}
          ...
          <div className="grid gap-4 sm:grid-cols-1">
            <label className="space-y-2">
              <span className="text-sm font-semibold text-textMuted uppercase">Cultivation Area (Acres) *</span>
              <input
        ...
        <p className="text-textSecondary text-sm pt-4 border-t border-cardBorder">No crops found for this farm.</p>
      )}
    </div>
  );
}
