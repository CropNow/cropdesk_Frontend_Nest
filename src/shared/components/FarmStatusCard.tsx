  ...
  return (
    ...
    >
      ...
      <div className="mb-0.5 flex items-baseline justify-center gap-0.5">
        <span className="text-base font-bold tracking-tight text-textHeading sm:text-lg lg:text-2xl">
          {metric.value}
        </span>
        {metric.unit && (
          <span className="text-xs font-medium text-textMuted sm:text-sm">
            {metric.unit}
          </span>
        )}
      </div>
      <p className="text-[10px] font-medium text-textHint sm:text-xs">
        {metric.label}
      </p>
    </motion.div>
  );
