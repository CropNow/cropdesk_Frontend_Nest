...
import { DEVICE_LIBRARY, DeviceType } from "@shared/constants/deviceConstants";
...
export function useDashboardState() {
    ...
    const fetchFarmData = async () => {
      try {
        ...
        const mappedMetrics = FARM_STATUS_METRICS.map((m) => {
          const liveMoisture =
            sensorLatestData?.values?.soil_moisture_1 ||
            sensorLatestData?.values?.soil_moisture ||
          ...
          const liveVisibility =
            stats?.currentConditions?.avgVisibility ??
            overview?.weather?.visibility ??
            overview?.weather?.visibilityKm ??
            0;

          if (m.id === "soil-moisture") {
            const val =
              liveMoisture !== undefined && liveMoisture !== null
                ? liveMoisture
                : stats?.currentConditions?.avgSoilMoisture;
          ...
          if (m.id === "visibility") {
            const val =
              liveVisibility !== undefined && liveVisibility !== null
                ? liveVisibility
                : 0;
            if (val !== undefined && val !== null) return { ...m, value: val };
          }
  ...
  const currentDevice =
    deviceList[currentDeviceIndex % (deviceList.length || 1)] || null;
