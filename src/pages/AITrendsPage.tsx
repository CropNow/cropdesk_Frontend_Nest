...
import { TrendingUp, BrainCircuit, Calendar, Filter, Newspaper, WifiOff, Clock } from 'lucide-react';
...
const SkeletonLoader = () => (
  <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
    {[1, 2, 3].map((n) => (
      <div key={n} className="animate-pulse rounded-[2rem] border border-cardBorder bg-cardBg/50 p-6 space-y-4">
              ...
              {news.map((item, index) => (
                ...
                >
                  <div className="space-y-3">
                    <span className={`inline-block rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${getBadgeStyles(item.category)}`}>
                      {item.category}
                    </span>
